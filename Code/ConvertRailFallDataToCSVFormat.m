...
for i=1:numtimesstep
    ...
    %RainFallAmt100(i,1)=RainFallAmt(i);
    irem=mod(i,100);
    if(irem==0)
        dispstr=strcat('Now processing row-',num2str(i),'-of-',num2str(numtimesstep100),'-total rows');
        disp(dispstr);
    end
    ab=2;
end
...
% dispstr=strcat('Now adding RainFallData To-',csvfilename);
% disp(dispstr)
% dlmwrite(csvfilename,RainFallAmt100,'-append','delimiter','\t','coffset',1,'precision','%10.5f')
