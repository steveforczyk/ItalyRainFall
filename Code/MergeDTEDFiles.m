...
Fipsvalue=5;% Fips Code for Tennessee
...
%% Save this file
    eval(['cd ' statedtedpath(1:length(statedtedpath)-1)]);
    MatFileName='UnifiedColoradoDTED.mat';
    actionstr='save';
    varstr1='Colorado ColoradoRB DTEDFiles latlim lonlim';
    qualstr='-v7.3';
    [cmdString]=MyStrcatV73(actionstr,MatFileName,varstr1,qualstr);
    eval(cmdString)
    dispstr=strcat('Saved State Level DEM File as-',MatFileName);
    disp(dispstr);
%% Now plot the data
    usamap(Colorado,ColoradoRB)
    geoshow(Colorado,ColoradoRB,DisplayType="surface")
    demcmap(Colorado)
    maxht=max(max(Colorado));
    dispstr=strcat('Max Elevation=',num2str(maxht));
    disp(dispstr)
    c = colorbar;
    c.Label.String = "Elevation (m)";
